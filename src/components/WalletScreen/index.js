...
import SettingsIcon from "./SettingsIconComponent";
...
class WalletScreen extends PureComponent {
  ...
  fetchTransactions() {
    ...
    OstJsonApi.getTransactionsForUserId(CurrentUser.getUserId(), 
                                        null,
                                        (res)=>{
                                          this.transactionUsers = res && res["transaction_users"] || this.transactionUsers;
                                          this.onResponse(res);
                                        },
                                        this.onError)
  }
  ...
  getNext = () => {
    if (
      ...
      !this.next_page_payload.pagination_identifier
    )
      return;
      OstJsonApi.getTransactionsForUserId(CurrentUser.getUserId(), 
  ...
  onResponse = (res) => {
    console.log("fetchTransactions===" , res);
    if (res.result_type) {
